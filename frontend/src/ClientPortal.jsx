...
export default function ClientPortal() {
  ...
  const checkCpf = async () => {
    ...
    try {
      ...
      try { localStorage.setItem(CPF_KEY, cpf); } catch {}
  ...
  const disconnect = () => {
    ...
    setPhone(""); setData(null); setScreen("home");
    ...
    return (
      <PortalShell>
        <div className="pt-card pt-connect">
          ...
          <a className="pt-link" href="/landing.html" style={{ display: "block", marginTop: "1.2rem" }}>← Voltar ao site</a>
        </div>
      </PortalShell>
    );
  }
  ...
  const doAbsence = async (b) => {
    setBusy(true);
    try {
      await api.portal.absence(phone, b.id, absenceText.trim());
  ...
  return (
    <PortalShell>
      ...
      {screen === "book" ? (
        <BookScreen data={data} busy={busy} onBack={() => setScreen("home")} onBook={doBook} />
...
function BookScreen({ data, busy, onBack, onBook }) {
  const byDay = {};
  data.available.forEach((s) => { (byDay[s.date] = byDay[s.date] || []).push(s); });
  const days = Object.keys(byDay).sort();
  return (<>
    <button className="pt-link" onClick={onBack}>← Voltar</button>
    <h2 className="pt-h2">Horários disponíveis</h2>
    <p className="pt-sub2" style={{ marginBottom: "1rem" }}>Toque no horário que você quer marcar.</p>
      ...
      <div className="pt-day" key={d}>
        ...
        {byDay[d].map((s) => (
          <button className="pt-slot" key={s.id} onClick={() => onBook(s)} disabled={busy}>
            <div><b>{s.time}</b><span> · {s.unit} · com {s.prof}</span></div>
